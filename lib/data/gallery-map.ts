...
export const getLocationImages = (locationSlug: string): GalleryImage[] => {
  ...
  return allImages.filter(image => 
    image.locationTags?.some(tag => 
      tag.toLowerCase().includes(locationSlug.toLowerCase())
    )
  );
};
...
export const getServiceImages = (serviceSlug: string): GalleryImage[] => {
  ...
  return allImages.filter(image => 
    ...
    image.tags?.some(tag => 
      tag.toLowerCase().includes(serviceSlug.toLowerCase())
    )
  );
};

// Helper function to get images for location + service combination
export const getLocationServiceImages = (locationSlug: string, serviceSlug: string): GalleryImage[] => {
  ...
  const uniqueImages = combinedImages.filter((image, index, self) => 
...
export const getPriorityServiceImages = (serviceSlug: string, count: number = 3): GalleryImage[] => {
  const images = getServiceImages(serviceSlug);
