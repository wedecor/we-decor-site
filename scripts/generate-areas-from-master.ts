  ...
  if (multiLineOnlySections.includes(label)) {
    ...
    if (label === 'FAQ') {
      ...
      if (process.argv.includes('--debug')) {
        ...
        console.log(`[DEBUG] FAQ source block preview:`, src.substring(src.indexOf('FAQ:'), src.indexOf('FAQ:') + 200));
      }
...
function parseLocalityBlock(block: string): LocalityData | null {
  try {
    ...
    if (!data.title || !data.meta || !data.intro || !data.services || 
        !data.why || !data.nearby || !data.faq || !data.caseStudy || 
        !data.cta || !data.locality) {
      console.warn(`⚠️  Missing required fields for locality: ${data.locality || 'Unknown'}`);
      console.warn(`   Missing: ${Object.entries(data).filter(([k, v]) => !v).map(([k]) => k).join(', ')}`);
      return null;
    }
