...
try {
  ...
  for (const slug of slugs.slice(0, 5)) { // Check first 5 for demo
    ...
    try {
      ...
      const hasGallery = html.includes('Recent Setups in') && html.includes('Browse our recent setups across');
      const hasGalleryImages = html.includes('group cursor-pointer bg-white rounded-lg overflow-hidden border border-gray-200');
