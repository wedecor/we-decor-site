...
function run() {
  ...
  const report = {
    ...
    summary: {
      ...
      uniquenessScore: Math.round(((GENERATED_LOCATIONS.length - pairs.length) / GENERATED_LOCATIONS.length) * 100)
    }
  };
  ...
  const avgBodyWords = Math.round(wordCounts.reduce((sum, l) => sum + l.bodyWords, 0) / wordCounts.length);
  const avgFaqs = Math.round(wordCounts.reduce((sum, l) => sum + l.faqCount, 0) / wordCounts.length);
