...
const dependencyAnalysis = {
  ...
  summary: {
    ...
    notes:
      'Next.js and React require major upgrades. Tailwind v4 migration planned as a follow-up.',
  },
  packages: [
    {
      name: 'next',
      current: process.env.NEXT_CURRENT || '14.1.0',
      latest: process.env.NEXT_LATEST || '15.5.2',
      status: 'critical',
      priority: 'P0',
      recommendation: 'Upgrade to 15.5.2 and complete App Router migration.',
    },
    {
      name: 'react',
      current: process.env.REACT_CURRENT || '18.2.0',
      latest: process.env.REACT_LATEST || '19.1.0',
      status: 'critical',
      priority: 'P0',
      recommendation: 'Upgrade to 19.1.0 along with react-dom.',
    },
    {
      name: 'react-dom',
      current: process.env.REACTDOM_CURRENT || '18.2.0',
      latest: process.env.REACTDOM_LATEST || '19.1.0',
      status: 'critical',
      priority: 'P0',
      recommendation: 'Upgrade with React.',
    },
    {
      name: 'tailwindcss',
      current: process.env.TW_CURRENT || '3.4.x',
      latest: process.env.TW_LATEST || '4.0.x',
      status: 'warning',
      priority: 'P2',
      recommendation: 'Plan migration to v4 in a separate PR.',
    },
    {
      name: 'typescript',
      current: process.env.TS_CURRENT || '5.8.3',
      latest: process.env.TS_LATEST || '5.9.x',
      status: 'warning',
      priority: 'P3',
      recommendation: 'Upgrade to latest 5.9.x.',
    },
  ],
};
  ...
  routes: [
    {
      ...
      open_graph: true,
      twitter: true,
      json_ld: ['LocalBusiness', 'BreadcrumbList', 'FAQPage'],
      issues: [],
    },
    {
      path: '/services',
    ...
    },
    {
      ...
      twitter: true,
      json_ld: ['BreadcrumbList'],
      issues: ['Alt text too generic on some images'],
    },
    {
      path: '/contact',
      score: 91,
    ...
    },
    {
      ...
      open_graph: true,
      twitter: true,
      json_ld: ['BreadcrumbList', 'FAQPage'],
      issues: [],
    },
    {
      path: '/locations',
...
writeCSV(
  ...
  [
    ...
    [
      ...
      1.8,
      1350,
      'Lightbox & filters shipped to client; consider dynamic import.',
    ],
  ...
  checks: [
    {
      ...
      count: 3,
      wcag: '1.4.3',
      notes: 'Brand gradient vs text contrast borderline on dark theme.',
    },
  ...
  hotspots: [
    {
      ...
      lines: 700,
      client: true,
      note: 'Split modal/lightbox; use dynamic import for client-only parts.',
    },
    {
      path: 'app/layout.tsx',
      client: false,
      note: 'Move analytics to next/script with lazy strategy where applicable.',
    },
  ],
  recommendations: [
