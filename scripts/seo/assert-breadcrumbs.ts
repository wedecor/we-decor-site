...
(async () => {
  ...
  const matches = Array.from(sitemap.text.matchAll(/<loc>([^<]*\/areas\/[\w-]+)<\/loc>/g)).map(m => m[1]);
  ...
  for (const url of sample) {
    ...
    const hasBreadcrumb = /"@type":"BreadcrumbList"/.test(page.text) && /"position":3/.test(page.text);
