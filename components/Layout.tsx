...
export default function Layout({ children, seo }: LayoutProps) {
  return (
    <>
      ...
      <main className="pt-20 pb-24 min-h-screen bg-gray-50 font-sans">{children}</main>
      {/* Footer with business info and Instagram link */}
      <Footer />
      {/* 🎨 UI Upgrade: Sticky WhatsApp CTA for instant messaging conversion */}
      <a
        href="https://wa.me/919999999999"
        className="fixed bottom-6 right-6 bg-gradient-to-r from-green-500 to-blue-500 text-white px-6 py-3 rounded-full shadow-xl z-50 flex items-center gap-3 hover:scale-105 transition transform duration-200 animate-pulse"
        target="_blank"
        rel="noopener"
        id="whatsapp-cta"
      ...
      {/* 🎨 UI Upgrade: Sticky Call Now CTA for instant phone conversion */}
      <a
        href="tel:+919999999999"
        className="fixed bottom-24 right-6 bg-gradient-to-r from-blue-600 to-green-500 text-white px-6 py-3 rounded-full shadow-xl z-50 flex items-center gap-3 hover:scale-105 transition transform duration-200 animate-pulse"
        id="callnow-cta"
      ...
      {/* 🎨 UI Upgrade: Privacy reassurance note for user trust and compliance */}
