  ...
  return (
    <section id={keyId} className="scroll-mt-24">
      <div className="mb-4 flex items-baseline justify-between gap-3">
        ...
        <Link href="#top" className="text-sm underline opacity-70 text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300">Back to top</Link>
      </div>
      ...
      <ul className="mb-4 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
        {areaSlugs.map(slug => {
          ...
          return (
            <li key={slug} className="rounded-2xl border border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-800 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between gap-3">
                <Link href={`/locations/${slug}`} className="font-medium hover:underline text-gray-900 dark:text-white hover:text-green-600 dark:hover:text-green-400">
                  {areaName}
                </Link>
                <Link href={`/locations/${slug}`} className="rounded-xl border border-green-200 dark:border-green-700 px-3 py-1 text-sm text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors">
                  View
                </Link>
              </div>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 md:py-12">
          ...
          <header className="mb-8 md:mb-12">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <a href={`tel:${PHONE_DISPLAY.replace(/\s/g, '')}`} className="rounded-xl border border-green-200 dark:border-green-700 px-4 py-2 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors">Call</a>
              <a href={`https://wa.me/918880544452?text=Hi! I need decoration services in ${CITY}`} target="_blank" className="rounded-xl border border-green-200 dark:border-green-700 px-4 py-2 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors">WhatsApp</a>
          ...
          {/* In-page TOC */}
          <nav aria-label="Clusters" className="mb-8 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-800 shadow-sm">
            <p className="mb-2 text-sm font-medium text-gray-900 dark:text-white">Jump to a region:</p>
            <ul className="flex flex-wrap gap-3">
              {CLUSTERS.map(c => (
                <li key={c.key}>
                  <a href={`#${c.key}`} className="rounded-full border border-gray-200 dark:border-gray-600 px-3 py-1 text-sm hover:bg-green-50 dark:hover:bg-green-900/20 text-gray-700 dark:text-gray-300 hover:text-green-700 dark:hover:text-green-400 transition-colors">
                    {c.title}
                  </a>
                </li>
              ))}
